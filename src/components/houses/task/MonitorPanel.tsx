...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  paused: { bg: 'bg-slate-500/20', text: 'text-slate-300', border: 'border-slate-500/30' },
  pending: { bg: 'bg-white/10', text: 'text-white/50', border: 'border-white/20' },
}
...
function StatCard({ 
  ...
  return (
    <div className={cn(
      ...
      <span className="text-[10px] font-mono text-white/40 uppercase tracking-wide">{label}</span>
    </div>
  )
}
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-mono text-white/90 truncate flex-1">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          {(task.status === 'interrupted' || task.status === 'terminated') && (
            <button
              onClick={onRetry}
              className="p-1 text-white/30 hover:text-violet-400 transition-colors"
              title="重试"
            >
...
function LogEntry({ log }: { log: ExecutionLogEntry }) {
  const style = EVENT_STYLES[log.event] || { icon: 'dot', color: 'text-white/30' }
  ...
  return (
    <div className="flex items-center gap-2 text-[11px] font-mono py-1 border-b border-white/5 last:border-0">
      <span className="text-white/30 w-16 flex-shrink-0">{time}</span>
      <span className={cn('w-3 h-3 flex items-center justify-center', style.color)}>
        {style.icon === 'check' && <CheckCircle2 className="w-3 h-3" />}
        {style.icon === 'x' && <XCircle className="w-3 h-3" />}
        {style.icon === 'play' && <Play className="w-3 h-3" />}
        {style.icon === 'pause' && <Pause className="w-3 h-3" />}
        {style.icon === 'rotate' && <RotateCcw className="w-3 h-3" />}
        {style.icon === 'arrow' && <span className="text-xs">→</span>}
        {style.icon === 'dot' && <span className="w-1.5 h-1.5 rounded-full bg-current" />}
      </span>
      <span className="text-white/60 truncate flex-1">{log.event.replace(/_/g, ' ')}</span>
      {displayData && (
        <span className="text-white/30 truncate max-w-[120px]">{displayData}</span>
      )}
    </div>
  )
}
...
export function MonitorPanel() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* 性能指标条 */}
      {stats && (
        <div className="flex items-center justify-between px-4 py-2 bg-white/5 border-b border-white/10 text-[11px] font-mono">
          ...
          <span className="text-white/40">
            总任务: <span className="text-white/60">{stats.totalTasks}</span>
          </span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {failedTasks.length > 0 && (
          <div className="mt-4">
            <div className="text-[11px] font-mono text-white/40 uppercase tracking-wide mb-2">
      ...
      <div className="border-t border-white/10">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-2 text-xs font-mono text-white/50 hover:bg-white/5 transition-colors"
        >
        ...
        {logsExpanded && (
          ...
          >
            {logs.length === 0 ? (
              <div className="text-center text-[11px] font-mono text-white/30 py-4">
                暂无执行日志
              </div>
            ) : (
