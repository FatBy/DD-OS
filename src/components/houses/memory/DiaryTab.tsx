/**
 * DiaryTab - MemoryHouse 日记 Tab
 *
 * 展示按天聚合的 AI 生成日记，支持展开详情、生成今日日记。
 */
...
type MoodType = 'productive' | 'learning' | 'casual' | 'challenging'

const MOOD_MAP: Record<MoodType, string> = {
  productive: '⚡',
  learning: '📚',
  casual: '☕',
  challenging: '🔥',
...
/** 根据日记内容推断 mood */
function inferMood(entry: DiaryEntry): MoodType {
  const text = entry.content.toLowerCase()
  if (entry.sessionCount >= 8 || /完成|实现|部署|上线|发布/.test(text)) return 'productive'
  if (/学习|研究|探索|了解|调研/.test(text)) return 'learning'
  if (/bug|错误|问题|失败|困难|修复/.test(text)) return 'challenging'
  return 'casual'
}
...
export function DiaryTab() {
  ...
  const loadDiaries = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[DiaryTab] Failed to load diaries:', error)
    } finally {
      setLoading(false)
    }
  }, [expandedId])
  ...
  const handleGenerateToday = useCallback(async () => {
    ...
    try {
      ...
      if (content) await loadDiaries()
    } catch (error) {
      console.warn('[DiaryTab] Failed to generate today diary:', error)
    } finally {
      setGenerating(false)
    }
  }, [loadDiaries])
    ...
    return (
      <div className="flex-1 flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-4 text-center max-w-sm">
          <div className="w-16 h-16 rounded-2xl bg-emerald-50 border border-emerald-200/60 flex items-center justify-center">
            <BookOpen className="w-8 h-8 text-emerald-300" />
          </div>
          <div>
            <h3 className="text-base font-semibold text-stone-600 mb-1">暂无日记</h3>
            <p className="text-xs text-stone-400 leading-relaxed">
              日记由 AI 从每天的对话记忆中自动提炼生成。进行有意义的对话后，系统会在次日自动生成日记。
            </p>
          </div>
          <button
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-stone-200/60 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-stone-700">{diaries.length} 篇日记</span>
        </div>
        <div className="flex items-center gap-2">
          {!hasTodayDiary && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg
                ...
                transition-colors disabled:opacity-50"
            >
              {generating ? <Loader2 className="w-3 h-3 animate-spin" /> : <PenLine className="w-3 h-3" />}
              生成今日日记
            </button>
          )}
          <button
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        {diaries.map((diary, index) => {
          ...
          const mood = inferMood(diary)
          const moodIcon = MOOD_MAP[mood]
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isExpanded ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-500',
                  )}
                >
                  <span className="text-base leading-none">{moodIcon}</span>
                  <span className="text-[9px] leading-none mt-0.5">{getWeekday(diary.date)}</span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-stone-400">{diary.date}</span>
                    {!!(diary.raw.metadata as Record<string, unknown> | undefined)?.isPartial && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-emerald-100 text-emerald-600">
                        进行中
                      </span>
                    )}
                  </div>
                    ...
                    <div className="px-4 pb-4 pt-1">
                      <div className="pl-[52px]">
                        ...
                        <div className="relative">
                          <div className="absolute -left-6 top-0 bottom-0 w-px bg-emerald-200/60" />
                          <Sparkles className="absolute -left-[29px] top-0 w-3.5 h-3.5 text-emerald-400" />
