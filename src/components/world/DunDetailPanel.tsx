...
// Tab 类型
type DetailTab = 'ability' | 'sop' | 'skills' | 'records' | 'artifacts' | 'knowledge'
...
function getDynamicConfig(dun: DunEntity | undefined) {
  ...
  return {
    ...
    typeLabelCity: dun.label || 'Building',
    // 使用 Tailwind 兼容的动态样式
    bgClass: '', // 将改用内联样式
    borderClass: '', // 将改用内联样式
    textClass: '', // 将改用内联样式
...
export function DunDetailPanel() {
  ...
  // 多会话系统 - 用于点击 Execute 时创建新的 Dun 会话
  ...
  const dun = selectedDunForPanel ? duns.get(selectedDunForPanel) : null
  ...
  }, [dun, skills, openClawSkills])

  // File-based Dun can execute even without bound skills (it has SOP)
  const canExecute = boundSkills.length > 0 || !!dun?.sopContent

  // 可用技能列表（排除已绑定的，按搜索词过滤）
  const availableSkillsForPicker = useMemo(() => {
    const boundIds = new Set((dun?.boundSkillIds || []).map(s => s.toLowerCase().trim()))
  ...
  }, [dun, skills, openClawSkills, skillSearchQuery])
  ...
  // 导入 Dun JSON
  const handleImportDun = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const dunConversations = useMemo(() => {
    ...
    return [...conversations.values()]
      .filter(c => c.type === 'dun' && c.dunId === dun.id)
  ...
  const handleSearchAndInstallSkill = async (skillName: string) => {
    ...
    try {
      ...
      if (installResult.success) {
        ...
        // 4. 自动绑定到当前 Dun：用安装后的真实名称替换原 unavailable 条目
        if (dun) {
          const oldIds = dun.boundSkillIds || []
  ...
  const activeTask = useMemo(() => {
    ...
    return allTasks.find(t => 
      t.status === 'executing' && 
      t.taskPlan?.dunId === dun.id
    ) || null
  }, [dun, activeExecutions, tasks])
  ...
  useEffect(() => {
    if (!dunPanelOpen) {
      ...
      setEditNameValue('')
    } else if (selectedDunForPanel) {
      // 如果有预填输入，自动切换到主聊天面板的 Dun 会话
      if (pendingDunChatInput) {
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      console.error('Failed to update dun name', e)
    }
  }
  ...
  return (
    ...
    <AnimatePresence>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <button onClick={closeDunPanel} className="p-1.5 text-stone-300 hover:text-stone-500 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                >
                  ...
                  <div className="w-48">
                    <div className="flex justify-between text-xs font-mono text-stone-400 mb-1">
                      <span>{t('dun.constructing_progress')}</span>
              ...
              {(() => {
                const species = (dun as any).species || getDefaultSpecies(dun.id)
              ...
              {/* ==================== Tab Bar ==================== */}
              <div className="flex border-b border-stone-200 -mx-6 px-6 overflow-x-auto scrollbar-hide">
                {TAB_CONFIG.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={cn(
                      'flex items-center gap-1 px-2.5 py-2 text-xs whitespace-nowrap transition-all border-b-2 -mb-px shrink-0',
                      activeTab === tab.id
              ...
              {(dun.objective || dun.metrics || dun.strategy) && (
                <div className="p-5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                  ...
                  {dun.objective && (
                    <div className="mb-4">
                      <p className="text-sm text-stone-700 leading-relaxed">{dun.objective}</p>
                    </div>
                  )}
                  ...
                  {dun.strategy && (
                    <div className="mb-3 p-3 rounded-lg bg-stone-50 border border-white/[0.04]">
                      ...
                      <p className="text-xs text-stone-500 leading-relaxed whitespace-pre-wrap">{dun.strategy}</p>
                    </div>
                  )}
                  
                  {dun.metrics && dun.metrics.length > 0 && (
              ...
              <div className="p-5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                ...
                {boundSkills.length > 0 ? (
                              ...
                              onClick={(e) => {
                                ...
                                fetch(`${serverUrl}/duns/${dun.id}/skills`, {
                                  ...
                                  .then(res => {
                                    ...
                                    // 后端无此 Dun (Observer 创建) → 前端直接移除
                                    const newSkills = (dun.boundSkillIds || []).filter(s => s !== skill.name)
                                    removeDun(dun.id)
                                    addDun({ ...dun, boundSkillIds: newSkills })
                                    addToast({ type: 'success', title: `已移除技能: ${skill.name}` })
                                    return null
                                  })
                                  .then(data => {
                                    if (!data) return // 已在上方前端处理
                                    const updated = { ...dun, boundSkillIds: data.skillDependencies || [] }
                                    removeDun(dun.id)
                                    addDun(updated)
                                    addToast({ type: 'success', title: `已移除技能: ${skill.name}` })
                                  })
                  ...
                  <p className="text-sm font-mono text-stone-300 italic">No skills bound to this Dun</p>
                )}
              </div>
                ...
                <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-mono text-amber-600">
                      ...
                      {dun.sopRewriteInfo.previousVersion && (
                        <span className="text-amber-400 ml-1">
                          ({dun.sopRewriteInfo.previousVersion} → {dun.version || 'latest'})
                        </span>
                      )}
                    </p>
              ...
              {dun.sopContent && (
                <div className="p-5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                  ...
                  <div className="max-h-[500px] overflow-y-auto">
                    <MarkdownRenderer content={dun.sopContent} className="text-sm text-stone-600" />
                  </div>
                </div>
              )}
              ...
              {!dun.sopContent && (
                <div className="py-12 text-center">
                  ...
                  <p className="text-xs font-mono text-stone-300 mt-1">此 Dun 尚未定义标准操作流程</p>
                </div>
              )}
              </>)}

              {/* ==================== Active Dun Indicator ==================== */}
              {activeDunId === dun.id && (
              ...
              {dunConversations.length > 0 && (
                <div className="p-5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="ml-auto text-xs font-mono text-stone-300">
                      {dunConversations.length}
                    </span>
                  </div>
                  <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
                    {dunConversations.map(conv => (
                      <button 
                        key={conv.id} 
              ...
              <div className="flex items-center gap-2 text-xs font-mono text-stone-300">
                ...
                <span>Created {new Date(dun.createdAt).toLocaleDateString()}</span>
                {dun.lastUsedAt && (
                  <>
                    <span className="text-stone-200">|</span>
                    <span>Last used {new Date(dun.lastUsedAt).toLocaleDateString()}</span>
                  </>
                )}
              </div>
              {/* ==================== TAB: RECORDS - end ==================== */}
              ...
              {/* ==================== TAB: ARTIFACTS ==================== */}
              {activeTab === 'artifacts' && (() => {
                const artifacts = dun?.id ? dunManager.getDunArtifacts(dun.id) : []
                return (
                  <div className="space-y-2">
                    {artifacts.length > 0 ? (
                      ...
                      <div className="py-12 text-center">
                        ...
                        <p className="text-xs font-mono text-stone-300 mt-1">Dun {'\u6267\u884C\u4EFB\u52A1\u4EA7\u751F\u7684\u6587\u4EF6\u5C06\u81EA\u52A8\u663E\u793A\u5728\u8FD9\u91CC'}</p>
                      </div>
                    )}
                  </div>
                )
      ...
      {activeTab === 'knowledge' && dun && (
        <DunKnowledgeTab dunId={dun.id} />
      )}
              </>
              )}            </div>
