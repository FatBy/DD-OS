...
export class NexusManager {
  ...
  private nexusDir(nexusId: string): string {
    return join(this.dataDir, "nexuses", nexusId);
  }

  private nexusMdPath(nexusId: string): string {
    return join(this.nexusDir(nexusId), "NEXUS.md");
  }

  private experienceDir(nexusId: string): string {
    return join(this.nexusDir(nexusId), "experience");
  }
  ...
  listNexuses(): string[] {
    const nexusesDir = join(this.dataDir, "nexuses");
    if (!existsSync(nexusesDir)) return [];
    return readdirSync(nexusesDir, { withFileTypes: true })
      .filter((d) => d.isDirectory())
      .map((d) => d.name);
  }

  loadNexusMeta(nexusId: string): NexusMeta | null {
    const mdPath = this.nexusMdPath(nexusId);
    if (!existsSync(mdPath)) return null;

    const content = readFileSync(mdPath, "utf-8");
    return this.parseNexusMd(nexusId, content);
  }

  saveNexusMd(nexusId: string, content: string): void {
    this.ensureDir(this.nexusDir(nexusId));
    writeFileSync(this.nexusMdPath(nexusId), content, "utf-8");
  }
  ...
  private parseNexusMd(nexusId: string, content: string): NexusMeta {
    const meta: NexusMeta = { id: nexusId, name: nexusId, sopContent: content };
    ...
    if (fmMatch) {
      ...
      meta.name = getString("name") || nexusId;
  ...
  parseSOP(nexusId: string, content: string): SOPPhase[] {
  ...
  createSOPTracker(nexusId: string, nexusLabel: string, content: string): SOPTracker {
    const phases = this.parseSOP(nexusId, content);
  ...
  buildSOPContext(nexusId: string, maxLength: number): string {
    ...
    // Nexus identity
    if (meta.objective) {
      parts.push(`[Nexus: ${meta.name}] Objective: ${meta.objective}`);
    }
  ...
  searchExperiences(nexusId: string, query: string, topK = 3): string[] {
    const results: string[] = [];

    for (const type of ["successes", "failures"] as const) {
      const filePath = join(this.experienceDir(nexusId), `${type}.md`);
  ...
  buildSOPDirective(
    ...
    // optional mode
    let choice = `[SOP 适用性评估 - ${tracker.nexusLabel}]\n`;
    choice += `当前 Nexus "${tracker.nexusLabel}" 有预定义的 SOP 流程（共 ${tracker.phases.length} 个阶段）。\n`;
  ...
  /**
   * 从 NEXUS.md 的 ## Rules 段落解析规则
   */
  parseRules(nexusId: string): NexusRule[] {
    const meta = this.loadNexusMeta(nexusId);
   ...
   */
  buildRulesContext(nexusId: string): string {
    const rules = this.parseRules(nexusId);
    if (rules.length === 0) return "";

    const parts = [`[Nexus Rules - Active]`];
  ...
  private xpFilePath(nexusId: string): string {
    return join(this.nexusDir(nexusId), "xp.json");
  }
  ...
  loadNexusXP(nexusId: string): { xp: number; level: number } {
    const filePath = this.xpFilePath(nexusId);
    if (!existsSync(filePath)) return { xp: 0, level: 1 };
    try {
      const raw = readFileSync(filePath, "utf-8");
      const data = JSON.parse(raw);
      const xp = typeof data.xp === "number" ? data.xp : 0;
      return { xp, level: NexusManager.xpToLevel(xp) };
    } catch {
      return { xp: 0, level: 1 };
    }
  }

  saveNexusXP(nexusId: string, xp: number): void {
    this.ensureDir(this.nexusDir(nexusId));
    const level = NexusManager.xpToLevel(xp);
    const data = { xp, level, updatedAt: Date.now() };
    writeFileSync(this.xpFilePath(nexusId), JSON.stringify(data, null, 2), "utf-8");
  }

  // ============================================
  // Nexus Profile (complete context for agent)
  // ============================================

  buildNexusProfile(nexusId: string): string {
    const meta = this.loadNexusMeta(nexusId);
    if (!meta) return "";

    const parts: string[] = [];
    parts.push(`[Active Nexus: ${meta.name}]`);
    ...
    const { xp, level } = this.loadNexusXP(nexusId);
    parts.push(`Experience: Lv${level} (XP: ${xp})`);

    const experiences = this.searchExperiences(nexusId, "", 3);
  ...
  private fitnessFilePath(nexusId: string): string {
    return join(this.nexusDir(nexusId), "sop-fitness.json");
  }

  loadSOPFitness(nexusId: string): ExtensionSOPFitness {
    const fp = this.fitnessFilePath(nexusId);
    if (!existsSync(fp)) {
      return {
        ema: 0.5,
        totalExecutions: 0,
  ...
  saveSOPFitness(nexusId: string, data: ExtensionSOPFitness): void {
    this.ensureDir(this.nexusDir(nexusId));
    data.lastUpdatedAt = Date.now();
    writeFileSync(this.fitnessFilePath(nexusId), JSON.stringify(data, null, 2), "utf-8");
  }
  ...
  updateSOPFitness(nexusId: string, traceSummary: TraceSummary): ExtensionSOPFitness {
  ...
  buildSOPImprovementHints(nexusId: string): string | null {
    const data = this.loadSOPFitness(nexusId);
    ...
    parts.push(`[SOP Execution Intelligence — ${nexusName}]`);
  ...
  /**
   * Write new SOP content back to NEXUS.md, preserving frontmatter.
   */
  writeSOPContent(nexusId: string, newSOPContent: string): boolean {
    const mdPath = this.nexusMdPath(nexusId);
   ...
   */
  resetSOPFitnessAfterRewrite(nexusId: string): void {
    const data = this.loadSOPFitness(nexusId);
  ...
  // ============================================
  // Nexus Catalog (for Agent-driven matching)
  // ============================================

  /**
   * Build a compact catalog of all available Nexuses for injection
   * when no Nexus is currently active.
   */
  buildNexusCatalog(): string | null {
    const nexusIds = this.listNexuses();
    if (nexusIds.length === 0) return null;

    const entries: string[] = [];
    for (const nid of nexusIds) {
      if (entries.length >= 15) break;
      const meta = this.loadNexusMeta(nid);
      if (!meta) continue;
      const { level } = this.loadNexusXP(nid);
    ...
    const header = [
      "[Available Nexus Workstations]",
      "No Nexus is active. Review the list below. If a Nexus matches this task,",
      "output <ACTIVATE_NEXUS>nexusId</ACTIVATE_NEXUS> to activate it.",
      "If none fits, proceed without a Nexus.",
      "",
    ];
